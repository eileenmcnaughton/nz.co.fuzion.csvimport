...
<div class="crm-block crm-form-block crm-import-summary-form-block">
  ...
  <div id="help">
    ...
    {if $unMatchCount }
      ...
      <p class="error">
        {ts 1=$downloadMismatchRecordsUrl}You can
          <a href="%1">Download Mismatched Participants</a>
          . You may then correct them, and import the new file with the corrected data.{/ts}
      </p>
    {/if}
    ...
    {if $invalidRowCount }
      ...
      <p class="error">
        {ts 1=$downloadErrorRecordsUrl}You can
          <a href="%1">Download Errors</a>
          . You may then correct them, and import the new file with the corrected data.{/ts}
      </p>
    {/if}
    ...
    {if $conflictRowCount}
      ...
      <p class="error">
        {ts 1=$downloadConflictRecordsUrl}You can
          <a href="%1">Download Conflicts</a>
          . You may then review these records to determine if they are actually conflicts, and correct the transaction IDs for those that are not.{/ts}
      </p>
    {/if}
    ...
    {if $duplicateRowCount}
      ...
      <p {if $dupeError}class="error"{/if}>
        {ts 1=$downloadDuplicateRecordsUrl}You can
          <a href="%1">Download Duplicates</a>
          . You may then review these records to determine if they are actually duplicates, and correct the transaction IDs for those that are not.{/ts}
      </p>
    {/if}
  </div>
  ...
  <table id="summary-counts" class="report">
    <tr>
      <td class="label">{ts}Total Rows{/ts}</td>
    ...
    {if $invalidRowCount }
      <tr class="error">
        <td class="label">{ts}Invalid Rows (skipped){/ts}</td>
    ...
    {if $unMatchCount }
      <tr class="error">
        <td class="label">{ts}Mismatched Rows (skipped){/ts}</td>
    ...
    {if $conflictRowCount}
      <tr class="error">
        <td class="label">{ts}Conflicting Rows (skipped){/ts}</td>
    ...
    <tr>
      <td class="label">{ts}Records Imported{/ts}</td>
